...
// /api/analyze.js — v3.8 PERFECT — Claude 4.5 Haiku Fixed + Anti-Hallucination + Cost Optimized
// Title: Insight Radar v3.8 PERFECT - Claude 4.5 Haiku | 2026-05-16
...
export default async function handler(req) {
  ...
  const prompt = `You are YouTube analyst for Insight Radar v3.8 PERFECT.
...
`;
  ...
  try{
    ...
    return new Response(JSON.stringify({analysis, model:'v3.8 PERFECT / haiku-4.5', version:'v3.8 PERFECT'}),{status:200,headers:{'content-type':'application/json','Cache-Control':'no-store'}});
  }catch(e){
    clearTimeout(to);
